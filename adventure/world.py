...
class World:
    ...
    def load_graph(self, room_graph):
        ...
        #     # grid_size = max(grid_size, room_graph[i][0][0], room_graph[i][0][1])
            self.rooms[i] = Room(i, island_map[str(i)]['room_title'], island_map[str(i)]['description'])
        self.room_grid = []
        grid_size += 1
        ...
        for room_id in room_graph:
            ...
            if 'n' in room_graph[room_id]:
                self.rooms[room_id].connect_rooms('n', self.rooms[room_graph[room_id]['n']])
            if 's' in room_graph[room_id]:
                self.rooms[room_id].connect_rooms('s', self.rooms[room_graph[room_id]['s']])
            if 'e' in room_graph[room_id]:
                self.rooms[room_id].connect_rooms('e', self.rooms[room_graph[room_id]['e']])
            if 'w' in room_graph[room_id]:
                self.rooms[room_id].connect_rooms('w', self.rooms[room_graph[room_id]['w']])
